...
export default function PremiumAppShell() {
  ...
  return (
    <>
      ...
      <style jsx global>{`
        body.kompasiAppMode{padding-bottom:calc(80px + env(safe-area-inset-bottom,0px))!important;overflow-x:hidden!important}
        ...
        .appDock{position:fixed;right:0;bottom:0;left:0;z-index:990;width:min(520px,100%);height:calc(80px + env(safe-area-inset-bottom,0px));margin:0 auto;padding:8px 9px max(9px,env(safe-area-inset-bottom,0px));display:grid;grid-template-columns:repeat(5,minmax(0,1fr));align-items:start;overflow:hidden;border:1px solid rgba(122,190,239,.58);border-bottom:0;border-radius:25px 25px 0 0;background:linear-gradient(118deg,rgba(7,67,125,.97),rgba(46,53,143,.97) 53%,rgba(5,112,91,.97));box-shadow:0 -12px 34px rgba(1,20,55,.3),inset 0 1px 0 rgba(255,255,255,.2);transform:none;isolation:isolate;backdrop-filter:blur(20px)}
        .appDock:before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 50% -30%,rgba(255,255,255,.28),transparent 42%);pointer-events:none}.appDock a{position:relative;z-index:1;min-width:0;height:60px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;border-radius:16px;color:#d8ecff;text-decoration:none;transition:background .16s ease,color .16s ease;transform:none}.navIcon{width:22px;height:22px;display:grid;place-items:center}.navIcon svg{width:22px;height:22px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.appDock a small{max-width:100%;overflow:hidden;color:inherit;font-size:9px;font-weight:900;letter-spacing:0;text-overflow:ellipsis;white-space:nowrap}.appDock a:nth-child(2){color:#e8dcff}.appDock a:nth-child(4){color:#c9ffeb}.appDock a:nth-child(5){color:#ffe9bc}.appDock a.active{background:rgba(255,255,255,.16);color:#fff;box-shadow:inset 0 1px 0 rgba(255,255,255,.18)}.appDock a.active::after{content:"";position:absolute;bottom:3px;width:19px;height:3px;border-radius:999px;background:#fff}.appDock a.primary{width:62px;height:60px;margin:0 auto;border:1px solid rgba(255,255,255,.5);border-radius:19px;background:linear-gradient(135deg,#23a9ff 0%,#7962ee 52%,#20c58c 100%);color:#fff;box-shadow:0 7px 20px rgba(0,15,48,.38),inset 0 1px 0 rgba(255,255,255,.42)}.appDock a.primary .navIcon{width:25px;height:25px}.appDock a.primary .navIcon svg{width:25px;height:25px}.appDock a.primary small{color:#fff;font-size:10px}.appDock a.primary.active{background:linear-gradient(135deg,#23a9ff,#7962ee 52%,#20c58c)}.appDock a.primary::after{display:none}.offlinePill{position:fixed;left:50%;top:max(10px,env(safe-area-inset-top));z-index:1100;padding:8px 13px;border:1px solid #f5d08c;border-radius:999px;background:#fff7e6;color:#925d06;box-shadow:0 9px 25px rgba(60,35,0,.16);font:800 11px/1.2 Arial,sans-serif;transform:translateX(-50%)}
