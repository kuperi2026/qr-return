...
export default function PremiumAppShell() {
  ...
  return (
    <>
      ...
      <style jsx global>{`
        body.kompasiAppMode{padding-bottom:calc(74px + env(safe-area-inset-bottom,0px))!important;overflow-x:hidden!important}
        ...
        .appDock{position:fixed;right:0;bottom:0;left:0;z-index:990;width:min(520px,100%);height:calc(74px + env(safe-area-inset-bottom,0px));margin:0 auto;padding:7px 8px max(7px,env(safe-area-inset-bottom,0px));display:grid;grid-template-columns:repeat(5,minmax(0,1fr));align-items:start;overflow:hidden;border:1px solid rgba(142,198,241,.78);border-bottom:0;border-radius:24px 24px 0 0;background:linear-gradient(115deg,#eaf7ff 0%,#f0edff 47%,#e7fff5 100%);box-shadow:0 -12px 32px rgba(1,25,62,.24),inset 0 1px 0 rgba(255,255,255,.95);transform:none;isolation:isolate}
        .appDock:before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 50% 0,rgba(104,73,223,.17),transparent 38%);pointer-events:none}.appDock a{position:relative;z-index:1;min-width:0;height:56px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;border-radius:15px;color:#4f6c89;text-decoration:none;transition:background .16s ease,color .16s ease;transform:none}.navIcon{width:22px;height:22px;display:grid;place-items:center}.navIcon svg{width:22px;height:22px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.appDock a small{max-width:100%;overflow:hidden;font-size:9px;font-weight:900;letter-spacing:0;text-overflow:ellipsis;white-space:nowrap}.appDock a:nth-child(2){color:#7353c7}.appDock a:nth-child(4){color:#078569}.appDock a:nth-child(5){color:#9a650f}.appDock a.active{background:rgba(255,255,255,.84);color:#075dce;box-shadow:0 5px 14px rgba(26,74,130,.13)}.appDock a.active::after{content:"";position:absolute;bottom:2px;width:18px;height:3px;border-radius:999px;background:linear-gradient(90deg,#0b72ed,#11a66d)}.appDock a.primary{width:62px;height:56px;margin:0 auto;border:2px solid rgba(255,255,255,.96);border-radius:19px;background:linear-gradient(135deg,#0b74e5 0%,#6950d8 52%,#13a66b 100%);color:#fff;box-shadow:0 8px 21px rgba(44,78,204,.38),inset 0 1px 0 rgba(255,255,255,.35)}.appDock a.primary .navIcon{width:25px;height:25px}.appDock a.primary .navIcon svg{width:25px;height:25px}.appDock a.primary small{color:#fff;font-size:10px}.appDock a.primary.active{background:linear-gradient(135deg,#0b74e5,#6950d8 52%,#13a66b)}.appDock a.primary::after{display:none}.offlinePill{position:fixed;left:50%;top:max(10px,env(safe-area-inset-top));z-index:1100;padding:8px 13px;border:1px solid #f5d08c;border-radius:999px;background:#fff7e6;color:#925d06;box-shadow:0 9px 25px rgba(60,35,0,.16);font:800 11px/1.2 Arial,sans-serif;transform:translateX(-50%)}
