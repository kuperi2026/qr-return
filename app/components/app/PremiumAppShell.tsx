...
export default function PremiumAppShell() {
  ...
  return (
    <>
      ...
      <style jsx global>{`
        body.kompasiAppMode{padding-bottom:calc(72px + env(safe-area-inset-bottom,0px))!important}
        ...
        .appDock{position:fixed;right:0;bottom:0;left:0;z-index:990;width:min(520px,100%);height:calc(72px + env(safe-area-inset-bottom,0px));margin:0 auto;padding:6px 10px max(6px,env(safe-area-inset-bottom,0px));display:grid;grid-template-columns:repeat(4,minmax(0,1fr));align-items:start;overflow:hidden;border:1px solid rgba(205,228,250,.9);border-bottom:0;border-radius:22px 22px 0 0;background:linear-gradient(145deg,rgba(255,255,255,.98),rgba(235,245,255,.97));box-shadow:0 -12px 32px rgba(1,25,62,.22),inset 0 1px 0 #fff;backdrop-filter:blur(24px) saturate(155%);transform:none;isolation:isolate}
        .appDock a{position:relative;min-width:0;height:54px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;border-radius:14px;color:#657d94;text-decoration:none;transition:none;transform:none}.navIcon{width:23px;height:23px;display:grid;place-items:center}.navIcon svg{width:23px;height:23px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.appDock a small{max-width:100%;overflow:hidden;font-size:10px;font-weight:900;letter-spacing:.02px;text-overflow:ellipsis;white-space:nowrap}.appDock a.active{background:#eaf3ff;color:#075dce}.appDock a.active::after{content:"";position:absolute;bottom:2px;width:20px;height:3px;border-radius:999px;background:linear-gradient(90deg,#0b72ed,#11a66d)}.appDock a.primary{width:58px;height:54px;margin:0 auto;border:2px solid rgba(255,255,255,.92);border-radius:16px;background:linear-gradient(145deg,#0b79ef 0%,#3158d8 54%,#6b47df 100%);color:#fff;box-shadow:0 7px 18px rgba(32,87,210,.32),inset 0 1px 0 rgba(255,255,255,.28)}.appDock a.primary .navIcon{width:24px;height:24px}.appDock a.primary .navIcon svg{width:24px;height:24px}.appDock a.primary small{color:#fff;font-size:10px}.appDock a.primary.active{background:linear-gradient(145deg,#0b79ef,#3158d8 54%,#6b47df)}.appDock a.primary::after{display:none}.offlinePill{position:fixed;left:50%;top:max(10px,env(safe-area-inset-top));z-index:1100;padding:8px 13px;border:1px solid #f5d08c;border-radius:999px;background:#fff7e6;color:#925d06;box-shadow:0 9px 25px rgba(60,35,0,.16);font:800 11px/1.2 Arial,sans-serif;transform:translateX(-50%)}
