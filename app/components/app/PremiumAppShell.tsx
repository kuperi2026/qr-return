  ...
  return (
    <>
      ...
      <style jsx global>{`
        ...
        .appDock{position:fixed;right:0;bottom:0;left:0;z-index:990;width:min(520px,100%);height:calc(74px + env(safe-area-inset-bottom,0px));margin:0 auto;padding:7px 8px max(7px,env(safe-area-inset-bottom,0px));display:grid;grid-template-columns:repeat(5,minmax(0,1fr));align-items:start;overflow:hidden;border:1px solid rgba(188,218,248,.95);border-bottom:0;border-radius:24px 24px 0 0;background:linear-gradient(115deg,#ffffff 0%,#edf7ff 38%,#f2edff 70%,#eafff7 100%);box-shadow:0 -10px 28px rgba(1,25,62,.2),inset 0 1px 0 #fff;transform:none;isolation:isolate}
        .appDock a{position:relative;min-width:0;height:56px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;border-radius:15px;color:#617b94;text-decoration:none;transition:none;transform:none}.navIcon{width:22px;height:22px;display:grid;place-items:center}.navIcon svg{width:22px;height:22px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.appDock a small{max-width:100%;overflow:hidden;font-size:9px;font-weight:900;letter-spacing:0;text-overflow:ellipsis;white-space:nowrap}.appDock a.active{background:rgba(255,255,255,.75);color:#075dce;box-shadow:0 5px 13px rgba(26,74,130,.1)}.appDock a.active::after{content:"";position:absolute;bottom:2px;width:18px;height:3px;border-radius:999px;background:linear-gradient(90deg,#0b72ed,#11a66d)}.appDock a.primary{width:62px;height:56px;margin:0 auto;border:2px solid rgba(255,255,255,.94);border-radius:18px;background:linear-gradient(145deg,#0b87ef 0%,#4655dc 52%,#7950df 100%);color:#fff;box-shadow:0 7px 18px rgba(44,78,204,.32),inset 0 1px 0 rgba(255,255,255,.3)}.appDock a.primary .navIcon{width:24px;height:24px}.appDock a.primary .navIcon svg{width:24px;height:24px}.appDock a.primary small{color:#fff;font-size:10px}.appDock a.primary.active{background:linear-gradient(145deg,#0b87ef,#4655dc 52%,#7950df)}.appDock a.primary::after{display:none}.offlinePill{position:fixed;left:50%;top:max(10px,env(safe-area-inset-top));z-index:1100;padding:8px 13px;border:1px solid #f5d08c;border-radius:999px;background:#fff7e6;color:#925d06;box-shadow:0 9px 25px rgba(60,35,0,.16);font:800 11px/1.2 Arial,sans-serif;transform:translateX(-50%)}
