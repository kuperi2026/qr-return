...
const quickScenarios: Record<string, string[]> = {
  dog: ["ძაღლი დაიკარგა", "ძაღლი იპოვეს", "ეშინია ან აგრესიულია"],
  cat: ["კატა დაიკარგა", "კატა იმალება", "კატა იპოვეს"],
  parking: ["გასასვლელი ჩაკეტილია", "ფარები დარჩა ჩართული", "ავტომობილი დაზიანებულია"],
  keys: ["გასაღებები ვიპოვე", "გასაღებები დავკარგე", "უსაფრთხო დაბრუნება"],
  wallet: ["საფულე ვიპოვე", "საფულე დავკარგე", "ბარათების დაცვა"],
  bag: ["ჩანთა ვიპოვე", "ჩანთა დავკარგე", "უსაფრთხო დაბრუნება"],
  suitcase: ["აეროპორტში დაიკარგა", "ჩემოდანი ვიპოვე", "ავიაკომპანიასთან კავშირი"],
  emergency: ["ადამიანი კონტაქტურია", "მესამე პირი მართავს პროფილს", "სასწრაფო საფრთხეა"],
};
...
export default function AppAiAssistant({ category, profileName, subjectMode, embedded = false }: Props) {
  ...
  const form = <form onSubmit={analyze}>
    ...
    {quickScenarios[activeCategory] && <div className="quickScenarios"><small>აირჩიეთ სიტუაცია ან აღწერეთ ქვემოთ</small><div>{quickScenarios[activeCategory].map((scenario) => <button type="button" key={scenario} onClick={() => setMessage(scenario)}>{scenario}</button>)}</div></div>}
