...
export default function PlansMenu({ka=true}:{ka?:boolean}) {
  ...
  return <section className="plansMenu"><div className="plansInner">
    <div className="plansHead"><div><span>QR RETURN SERVICE</span><h2>{ka?"მომსახურება და პაკეტები":"Service & plans"}</h2><p>{ka?"ყველა ახალი QR პროფილი გააქტიურებიდან პირველი 2 თვე უფასოდ მუშაობს. დაამატეთ თითოეული პროფილი და მიუთითეთ სასურველი ვადა.":"Every new QR profile includes two free months. Add each profile and choose its own period."}</p></div><div className="freeBadge"><strong>2 თვე</strong><small>{ka?"უფასოდ":"free"}</small></div></div>
    <section className="ownedPlans" aria-label={ka?"ჩემი შექმნილი პროფილები":"My profiles"}>
      <div className="ownedPlansHead"><div><h3>{ka?"ჩემი შექმნილი პროფილები":"My existing profiles"}</h3><p>{ka?"აქ ჩანს თქვენი ანგარიშით შექმნილი QR პროფილები. თითოეულისთვის მომსახურების ვადას ცალკე აირჩევთ.":"Profiles already created in your account. Choose a separate term for each."}</p></div><Link href="/account/subscriptions">{ka?"პროფილებზე ვადის არჩევა →":"Choose terms →"}</Link></div>
      {accountState==="loading"?<p className="ownedPlansState">{ka?"პროფილები იტვირთება...":"Loading profiles..."}</p>:accountState==="guest"?<p className="ownedPlansState">{ka?"თქვენი პროფილების სანახავად შედით ანგარიშში.":"Sign in to see your profiles."} <Link href="/login">{ka?"შესვლა →":"Sign in →"}</Link></p>:accountState==="error"?<p className="ownedPlansState">{ka?"პროფილების ჩატვირთვა ვერ მოხერხდა.":"Could not load your profiles."}</p>:ownedProfiles.length?<div className="ownedPlansGrid">{ownedProfiles.map(profile=>{const product=profileProduct(profile);return <article key={profile.id}><span className="ownedIcon">{product?.icon||"🏷️"}</span><div><strong>{profile.item_name||product?.name||profile.tag_code}</strong><small>{product?.name||"QR პროფილი"} · {profile.tag_code}</small></div><Link href={`/account/subscriptions?profile=${profile.id}`}>{ka?"ვადის არჩევა":"Choose term"}</Link></article>})}</div>:<p className="ownedPlansState">{ka?"ჯერ QR პროფილი არ შეგიქმნიათ.":"You have not created a QR profile yet."} <Link href="/register">{ka?"პროფილის შექმნა →":"Create profile →"}</Link></p>}
    </section>
    <div className="estimateHeading"><h3>{ka?"ფასის წინასწარი კალკულატორი":"Price estimator"}</h3><p>{ka?"სცადეთ ნებისმიერი პროდუქტი და რაოდენობა. ეს არჩევანი მხოლოდ ფასის გასაგებადაა და შეკვეთას არ აგზავნის.":"Try any product and quantity. This is only an estimate and does not place an order."}</p></div>
