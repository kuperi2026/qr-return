...
export default function PlansMenu({ka=true}:{ka?:boolean}) {
  ...
  return <section className="plansMenu"><div className="plansInner">
    ...
    <section className="ownedPlans" aria-label={ka?"ჩემი შექმნილი პროფილები":"My profiles"}>
      ...
      {accountState==="loading"?<p className="ownedPlansState">{ka?"პროფილები იტვირთება...":"Loading profiles..."}</p>:accountState==="guest"?<p className="ownedPlansState">{ka?"თქვენი პროფილების სანახავად შედით ანგარიშში.":"Sign in to see your profiles."} <Link href="/login">{ka?"შესვლა →":"Sign in →"}</Link></p>:accountState==="error"?<p className="ownedPlansState">{ka?"პროფილების ჩატვირთვა ვერ მოხერხდა.":"Could not load your profiles."}</p>:ownedProfiles.length?<div className="ownedPlansGrid">{ownedProfiles.slice(0,4).map(profile=>{const product=profileProduct(profile);return <article key={profile.id}><span className="ownedIcon">{product?.icon||"🏷️"}</span><div><strong>{profile.item_name||product?.name||profile.tag_code}</strong><small>{product?.name||"QR პროფილი"} · {profile.tag_code}</small></div><Link href={`/account/subscriptions?profile=${profile.id}`}>{ka?"ვადის არჩევა":"Choose term"}</Link></article>})}{ownedProfiles.length>4&&<Link className="allProfilesLink" href="/account/subscriptions">{ka?`ყველა პროფილი (${ownedProfiles.length}) →`:`All profiles (${ownedProfiles.length}) →`}</Link>}</div>:<p className="ownedPlansState">{ka?"ჯერ QR პროფილი არ შეგიქმნიათ.":"You have not created a QR profile yet."} <Link href="/register">{ka?"პროფილის შექმნა →":"Create profile →"}</Link></p>}
    </section>
