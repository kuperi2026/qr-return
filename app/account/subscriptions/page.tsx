    ...
    <section className="shell">
      ...
      <nav className="appCheckoutSteps" aria-label="პაკეტის გააქტიურების ეტაპები">
        {([1, 2, 3] as const).map((step) => (
          <button key={step} type="button" className={appStep === step ? "active" : appStep > step ? "done" : ""} onClick={() => setAppStep(step)}>
            ...
            <b>{step === 1 ? "პროფილი" : step === 2 ? "კალკულატორი" : "შეჯამება"}</b>
          </button>
        ))}
      </nav>
      ...
      <div className="layout">
        <section className="panel">
          ...
          <button type="button" className="appNext" disabled={!selectedProfiles.length} onClick={() => setAppStep(2)}>ფასის გამოთვლა <span>→</span></button>
          </div>
