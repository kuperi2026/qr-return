      ...
      <div className="layout">
        <section className="panel">
          ...
          <section className="liveCalculator" aria-label="ფასის კალკულატორი">
            <header><div><small>ფასი და პირობები</small><h3>ფასების ინსტრუქცია</h3></div><div><span>{selectedItems.length} არჩეული</span><strong>{selectedItems.length ? `${total} ₾` : "—"}</strong></div></header>
            <details className="priceInstructions"><summary><span>ყველა პროდუქტის ფასი</span><b>ნახვა ⌄</b></summary><div className="priceCalendar"><div className="priceCalendarHead"><span>პროდუქტი</span>{PERIODS.map((item) => <b key={item.value}>{item.label}</b>)}</div>{PRODUCTS.map((product) => { const chosen = selectedItems.filter((profile) => normalizeType(profile) === product.type); return <article key={product.type} className={chosen.length ? "chosenProduct" : ""}><span><i>{product.icon}</i><strong>{product.name}</strong>{chosen.length > 0 && <em>✓ {chosen.length}</em>}</span>{PERIODS.map((item) => <b key={item.value}>{product.prices[item.value]}₾</b>)}</article>; })}<p><strong>რაოდენობრივი ფასდაკლება:</strong> 3–4 პროფილი −5% · 5–6 −10% · 7 −12% · 8 ან მეტი −15%</p></div></details>
          </section>
