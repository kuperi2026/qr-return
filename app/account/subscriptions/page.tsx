      ...
      <div className="layout">
        <section className="panel">
          ...
          <section className="liveCalculator" aria-label="ფასის კალკულატორი">
            <header><div><small>ფასების ინსტრუქცია</small><h3>აირჩიეთ ვადა</h3></div><div><span>{selectedItems.length} პროფილი</span><strong>{selectedItems.length ? `${total} ₾` : "—"}</strong></div></header>
            ...
            <details className="priceInstructions"><summary>როგორ ითვლება ფასი და ფასდაკლება? <span>⌄</span></summary><div><p>თითოეული არჩეული პროფილის ფასი ემატება საერთო თანხას. ვადის შეცვლისას ჯამი ავტომატურად განახლდება.</p><p><b>ფასდაკლება:</b> 3–4 პროფილი −5% · 5–6 −10% · 7 −12% · 8 ან მეტი −15%</p></div></details>
          </section>
          {loading ? <div className="state">პროფილები იტვირთება...</div> : profiles.length ? <div className="profilePicker">
            ...
            {profileGroups.map((group) => group.type === openCategory && <section key={group.type} className="activeProfiles" role="tabpanel">
              ...
              <div className="profiles">{group.profiles.map((profile) => {
                ...
                return <button key={profile.id} className={selected ? "profile selected" : "profile"} onClick={() => toggleProfile(profile.id)}><span className="icon">{meta?.icon || "🏷️"}</span><span><b>{profile.item_name || meta?.name || "QR პროფილი"}</b><small>{meta?.name} · {profile.tag_code}</small></span><span className="profilePrice">{meta?.prices[period] || 0} ₾<small>{PERIODS.find((item) => item.value === period)?.label}</small></span><i>{selected ? "✓" : ""}</i></button>;
              })}</div>
            </section>)}
          </div> : <div className="state">ჯერ QR პროფილი არ გაქვს. <Link href="/register">პროფილის დამატება</Link></div>}
          <button type="button" className="appNext" disabled={!selectedProfiles.length} onClick={() => setAppStep(3)}>შეჯამება <span>→</span></button>
          </div>
        ...
        <aside className="summary checkoutStage summaryStage">
          ...
          <div className="summaryPeriodHeading"><span>მომსახურების ვადა</span><small>შეცვალეთ აქვე</small></div>
          <div className="summaryPeriods">{PERIODS.map((item) => <button key={item.value} type="button" aria-pressed={period === item.value} className={period === item.value ? "active" : ""} onClick={() => setPeriod(item.value)}><span>{item.label}</span><b>{selectedItems.reduce((amount, profile) => amount + (PRODUCTS.find((product) => product.type === normalizeType(profile))?.prices[item.value] || 0), 0)} ₾</b></button>)}</div>
