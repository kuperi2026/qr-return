...
export default function SubscriptionsPage() {
  ...
  return <main className="subscriptionsPage" data-app-step={appStep}>
    ...
    <section className="shell">
      ...
      <div className="layout">
        <section className="panel">
          ...
          {loading ? <div className="state">პროფილები იტვირთება...</div> : profiles.length ? <div className="profileGroups">{profileGroups.map((group) => {
            const expanded = openCategory === group.type;
            const selectedCount = group.profiles.filter((profile) => selectedProfiles.includes(profile.id)).length;
            return <section key={group.type} className={expanded ? "profileGroup open" : "profileGroup"}>
              <button type="button" className="profileGroupToggle" aria-expanded={expanded} onClick={() => setOpenCategory(expanded ? "" : group.type)}>
                <span className="groupIcon">{group.icon}</span><span className="groupTitle"><b>{group.label}</b><small>{group.profiles.length} პროფილი{selectedCount ? ` · ${selectedCount} არჩეული` : ""}</small></span><span className="groupChevron">⌄</span>
              </button>
              {expanded && <div className="profiles">{group.profiles.map((profile) => {
          ...
          <div className="periods">{PERIODS.map((item) => { const sum = selectedItems.reduce((amount, profile) => amount + (PRODUCTS.find((p) => p.type === normalizeType(profile))?.prices[item.value] || 0), 0); return <button key={item.value} className={period === item.value ? "period active" : "period"} onClick={() => setPeriod(item.value)}><b>{item.label}</b><span>{selectedItems.length ? `${sum} ₾` : "—"}</span></button>; })}</div>
    ...
    <style jsx>{`
      .appCheckoutSteps,.appNext,.appStageActions,.summaryPrevious,.appPriceCatalog{display:none}
      ...
      .profileGroups{display:grid;gap:10px}.profileGroup{overflow:hidden;border:1px solid #dce6f0;border-radius:15px;background:#fff}.profileGroup.open{border-color:#9fc9f4;box-shadow:0 7px 18px rgba(19,83,145,.08)}.profileGroupToggle{width:100%;min-height:68px;padding:11px 14px;display:flex;align-items:center;gap:12px;border:0;background:#f5f9fe;color:#17324d;text-align:left;cursor:pointer;font-family:inherit}.profileGroup.open .profileGroupToggle{background:#eaf4ff}.groupIcon{width:42px;height:42px;display:grid;place-items:center;border-radius:12px;background:#fff;font-size:22px;box-shadow:0 3px 10px rgba(11,67,119,.08)}.groupTitle{min-width:0;flex:1}.groupTitle b,.groupTitle small{display:block}.groupTitle b{font-size:16px}.groupTitle small{margin-top:4px;color:#687d91;font-size:12px;font-weight:700}.groupChevron{width:30px;height:30px;display:grid;place-items:center;border-radius:50%;background:#fff;color:#0871df;font-size:22px;font-weight:900;transition:transform .2s ease}.profileGroup.open .groupChevron{transform:rotate(180deg)}.profileGroup .profiles{padding:10px;border-top:1px solid #dceaf7;background:#fafdff}
