...
export default function SubscriptionsPage() {
  ...
  return <main className="subscriptionsPage" data-app-step={appStep}>
    ...
    <section className="shell">
      ...
      <div className="intro"><div><small>მომსახურება და პაკეტები</small><h1>გააგრძელეთ თქვენი QR პროფილის მომსახურება</h1><p>QR პროფილის გააქტიურებიდან პირველი 2 თვე უფასოა. ფასიანი მომსახურება იწყება უფასო პერიოდის დასრულების შემდეგ.</p></div><div className="free"><b>პირველი 2 თვე</b><span>გააქტიურებიდან უფასოა</span></div></div>
        ...
        <section className="panel">
          ...
          {loading ? <div className="state">პროფილები იტვირთება...</div> : profiles.length ? <div className="profilePicker">
            ...
            {profileGroups.map((group) => {
              ...
              const categoryProduct = PRODUCTS.find((item) => item.type === group.type);
              ...
              {categoryProduct && <details className="categoryPricing"><summary><span><b>ფასი და პირობები</b><small>{categoryProduct.prices[period]} ₾ · {PERIODS.find((item) => item.value === period)?.label}</small></span><i>⌄</i></summary><div><div className="categoryPeriods">{PERIODS.map((item) => <button key={item.value} type="button" className={period === item.value ? "active" : ""} onClick={() => setPeriod(item.value)}><span>{item.label}</span><b>{categoryProduct.prices[item.value]} ₾</b></button>)}</div><p><b>პირველი 2 თვე უფასოა</b> QR პროფილის გააქტიურებიდან. ფასიანი პერიოდი იწყება ამის შემდეგ.</p><p>რამდენიმე პროფილის არჩევისას ფასები ჯამდება: 3–4 პროფილი −5% · 5–6 −10% · 7 −12% · 8 ან მეტი −15%.</p></div></details>}
