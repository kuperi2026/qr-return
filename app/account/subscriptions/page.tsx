      ...
      <div className="layout">
        <section className="panel">
          ...
          <section className="liveCalculator" aria-label="ფასის კალკულატორი">
            ...
            <details className="priceInstructions"><summary>ფასების ინსტრუქცია <span>⌄</span></summary><div><p>QR კოდის გააქტიურების შემდეგ მომსახურებით სარგებლობა 60 დღის განმავლობაში უფასოა. ფასიანი პერიოდი იწყება 60 დღის დასრულების შემდეგ.</p><p><b>რამდენიმე პროფილის ფასდაკლება:</b> 3–4 პროფილი −5% · 5–6 −10% · 7 −12% · 8 ან მეტი −15%</p></div></details>
          </section>
          {loading ? <div className="state">პროფილები იტვირთება...</div> : profiles.length ? <div className="profilePicker">
            ...
            {profileGroups.map((group) => {
              ...
              <div className="profiles">{group.profiles.map((profile) => {
                ...
                return <article key={profile.id} className={selected ? "profileOption selected" : "profileOption"}><button className={selected ? "profile selected" : "profile"} onClick={() => toggleProfile(profile.id)}><span className="icon">{meta?.icon || "🏷️"}</span><span><b>{profile.item_name || meta?.name || "QR პროფილი"}</b><small>{meta?.name} · {profile.tag_code}</small></span><span className="profilePrice">{meta?.prices[period] || 0} ₾<small>{PERIODS.find((item) => item.value === period)?.label}</small></span><i>{selected ? "✓" : ""}</i></button>{selected && <div className="profileTerms"><div><span>არჩეული ვადა</span><b>{PERIODS.find((item) => item.value === period)?.label}</b></div><div><span>პროფილის ფასი</span><b>{meta?.prices[period] || 0} ₾</b></div><p>QR კოდის გააქტიურების შემდეგ მომსახურებით სარგებლობა 60 დღის განმავლობაში უფასოა.</p></div>}</article>;
              })}</div>
            </section>;})}
