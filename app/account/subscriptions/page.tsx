...
export default function SubscriptionsPage() {
  ...
  useEffect(() => { void (async () => {
    ...
    setIsAppPricing(params.get("source") === "app" || params.get("app_preview") === "1" || window.localStorage.getItem("kompasi-app-mode") === "1");
    ...
    setProfiles(rows); setSelectedProfiles(initialProfile?.id ? [initialProfile.id] : []);
  ...
  const selectedPeriod = (profileId: number) => isAppPricing ? (profilePeriods[profileId] || period) : period;
  const subtotal = useMemo(() => selectedItems.reduce((sum, profile) => sum + (PRODUCTS.find((item) => item.type === normalizeType(profile))?.prices[isAppPricing ? (profilePeriods[profile.id] || period) : period] || 0), 0), [selectedItems, period, profilePeriods, isAppPricing]);
  ...
  return <main className="subscriptionsPage" data-app-step={appStep}>
    ...
    <section className="shell">
      ...
      <div className="intro"><div><small>მომსახურება და პაკეტები</small><h1>გააგრძელეთ თქვენი QR პროფილის მომსახურება</h1><p>QR კოდის გააქტიურების შემდეგ მომსახურებით სარგებლობა 60 დღის განმავლობაში უფასოა.</p></div><div className="free"><b>60 დღე უფასოდ</b><span>QR კოდის გააქტიურების შემდეგ</span></div></div>
      ...
      <div className="layout">
        <section className="panel">
          ...
          <section className="liveCalculator" aria-label="ფასის კალკულატორი">
            <header><div><small>ფასი და პირობები</small><h3>ფასის კალკულატორი</h3></div><div><span>{selectedItems.length} პროფილი</span><strong>{selectedItems.length ? `${total} ₾` : "—"}</strong></div></header>
            <div className="calculatorPeriods">{PERIODS.map((item) => { const sum = selectedItems.reduce((amount, profile) => amount + (PRODUCTS.find((product) => product.type === normalizeType(profile))?.prices[item.value] || 0), 0); return <button key={item.value} type="button" className={period === item.value ? "active" : ""} onClick={() => setPeriod(item.value)}><span>{item.label}</span><b>{selectedItems.length ? `${sum} ₾` : "—"}</b></button>; })}</div>
            <details className="priceInstructions"><summary>ფასების ინსტრუქცია <span>⌄</span></summary><div><p className="repeatedFreePeriod">QR კოდის გააქტიურების შემდეგ მომსახურებით სარგებლობა 60 დღის განმავლობაში უფასოა. ფასიანი პერიოდი იწყება 60 დღის დასრულების შემდეგ.</p><p><b>რამდენიმე პროფილის ფასდაკლება:</b> 3–4 პროფილი −5% · 5–6 −10% · 7 −12% · 8 ან მეტი −15%</p></div></details>
          </section>
          {loading ? <div className="state">პროფილები იტვირთება...</div> : profiles.length ? <div className="profilePicker">
            ...
            {profileGroups.map((group) => {
              ...
              <div className="profiles">{group.profiles.map((profile) => {
                ...
                return <article key={profile.id} className={selected ? "profileOption selected" : "profileOption"}><button className={selected ? "profile selected" : "profile"} onClick={() => toggleProfile(profile.id)}><span className="icon">{meta?.icon || "🏷️"}</span><span><b>{profile.item_name || meta?.name || "QR პროფილი"}</b><small>{meta?.name} · {profile.tag_code}</small></span><span className="profilePrice">{meta?.prices[ownPeriod] || 0} ₾<small>{PERIODS.find((item) => item.value === ownPeriod)?.label}</small></span><i>{selected ? "✓" : ""}</i></button>{selected && <div className="profileTerms"><div className="profilePeriodPicker">{PERIODS.map((item) => <button key={item.value} type="button" className={ownPeriod === item.value ? "active" : ""} onClick={() => setProfilePeriods((current) => ({ ...current, [profile.id]: item.value }))}><span>{item.label}</span><b>{meta?.prices[item.value] || 0} ₾</b></button>)}</div><div className="desktopProfileTerm"><span>არჩეული ვადა</span><b>{PERIODS.find((item) => item.value === ownPeriod)?.label}</b></div><div className="desktopProfileTerm"><span>პროფილის ფასი</span><b>{meta?.prices[ownPeriod] || 0} ₾</b></div><p className="repeatedFreePeriod">QR კოდის გააქტიურების შემდეგ მომსახურებით სარგებლობა 60 დღის განმავლობაში უფასოა.</p></div>}</article>;
              })}</div>
            </section>;})}
          </div> : <div className="state">ჯერ QR პროფილი არ გაქვს. <Link href="/register">პროფილის დამატება</Link></div>}
          <button type="button" className="appNext" disabled={!selectedProfiles.length} onClick={() => setAppStep(3)}>შეჯამება · {selectedItems.length ? `${total} ₾` : "—"} <span>→</span></button>
          </div>
        ...
        <aside className="summary checkoutStage summaryStage">
          ...
          {selectedItems.length > 0 && <div className="chosen">{selectedItems.map((profile) => { const meta = PRODUCTS.find((item) => item.type === normalizeType(profile)); const ownPeriod = selectedPeriod(profile.id); return <div key={profile.id}><span>{meta?.icon} {profile.item_name || meta?.name}<small>{PERIODS.find((item) => item.value === ownPeriod)?.label}</small></span><b>{meta?.prices[ownPeriod] || 0} ₾</b></div>; })}</div>}
    ...
    <style jsx>{`
      ...
      .liveCalculator{margin:0 0 13px;padding:12px;border:1px solid #bcd9f2;border-radius:16px;background:linear-gradient(145deg,#fff,#eff7ff);box-shadow:0 9px 23px rgba(8,78,143,.11)}.liveCalculator>header{display:flex;align-items:center;justify-content:space-between;gap:10px}.liveCalculator>header small{color:#6d8498;font-size:8px;font-weight:900}.liveCalculator h3{margin:2px 0 0;color:#173652;font-size:16px}.liveCalculator>header>div:last-child{text-align:right}.liveCalculator>header>div:last-child span,.liveCalculator>header>div:last-child strong{display:block}.liveCalculator>header>div:last-child span{color:#6f879c;font-size:9px}.liveCalculator>header>div:last-child strong{margin-top:2px;color:#0870d8;font-size:20px}.calculatorPeriods{margin-top:10px;display:grid;grid-template-columns:repeat(4,1fr);gap:6px}.calculatorPeriods button{min-width:0;min-height:52px;padding:6px 2px;border:1px solid #d5e3ef;border-radius:10px;background:#fff;color:#526d85;font-family:inherit}.calculatorPeriods span,.calculatorPeriods b{display:block}.calculatorPeriods span{font-size:9px}.calculatorPeriods b{margin-top:4px;color:#0870d8;font-size:13px}.calculatorPeriods button.active{border-color:#0b74dd;background:#0b74dd;color:#fff;box-shadow:0 6px 14px rgba(11,116,221,.2)}.calculatorPeriods button.active b{color:#fff}.priceInstructions{margin-top:9px;border-top:1px solid #dce8f3}.priceInstructions>summary{min-height:38px;display:flex;align-items:center;justify-content:space-between;color:#456984;cursor:pointer;list-style:none;font-size:10px;font-weight:850}.priceInstructions>summary::-webkit-details-marker{display:none}.priceInstructions>div{padding:0 2px 4px}.priceInstructions p{margin:5px 0;color:#647d92;font-size:9px;line-height:1.45}.profilePrice{flex:0 0 auto;color:#0870d8;font-size:12px;font-weight:950;text-align:right}.profilePrice small{margin-top:2px!important;color:#7890a4!important;font-size:8px!important}.profile:not(.selected)>i{background:#e7eff7}.profile.selected .profilePrice{color:#07845a}
      ...
      .profilePeriodPicker{display:none}.chosen span small{display:block;margin-top:3px;color:inherit;font-size:9px;opacity:.72}
