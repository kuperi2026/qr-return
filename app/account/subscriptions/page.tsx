    ...
    <section className="shell">
      ...
      <div className="layout">
        <section className="panel">
          ...
          <button type="button" className="calculatorLauncher" onClick={() => setCalculatorOpen(true)}><span className="calculatorMark">₾</span><span><small>ფასების ინსტრუქცია</small><b>ფასის კალკულატორი</b><em>{selectedItems.length ? `${selectedItems.length} პროფილი · ${PERIODS.find((item) => item.value === period)?.label}` : "აირჩიეთ პროფილი და ვადა"}</em></span><strong>{selectedItems.length ? `${total} ₾` : "გახსნა"}<i>›</i></strong></button>
          {calculatorOpen && <div className="calculatorOverlay" role="presentation" onClick={() => setCalculatorOpen(false)}>
            <section className="calculatorSheet" role="dialog" aria-modal="true" aria-label="ფასის კალკულატორი" onClick={(event) => event.stopPropagation()}>
              <div className="sheetHandle" />
              <header><div><small>ფასების ინსტრუქცია</small><h3>ფასის კალკულატორი</h3></div><button type="button" onClick={() => setCalculatorOpen(false)} aria-label="დახურვა">×</button></header>
              <div className="inlineCalculatorBody">
              <p>მონიშნეთ პროფილი და აირჩიეთ ვადა — თანხა ავტომატურად გადაითვლება.</p>
              <div className="periods">{PERIODS.map((item) => { const sum = selectedItems.reduce((amount, profile) => amount + (PRODUCTS.find((product) => product.type === normalizeType(profile))?.prices[item.value] || 0), 0); const active = period === item.value; return <button key={item.value} type="button" className={active ? "period active" : "period"} aria-pressed={active} onClick={() => setPeriod(item.value)}><i aria-hidden="true">{active ? "✓" : ""}</i><small>მომსახურების ვადა</small><b>{item.label}</b><span>{selectedItems.length ? `${sum} ₾` : "—"}</span></button>; })}</div>
              <details className="calculatorPriceGuide"><summary>ყველა პროდუქტის ფასი <span>⌄</span></summary><div>{PRODUCTS.map((item) => <article key={item.type}><b>{item.icon} {item.name}</b><span>{item.prices[period]} ₾</span></article>)}</div></details>
              <div className="sheetTotal"><span>არჩეული ღირებულება</span><strong>{selectedItems.length ? `${total} ₾` : "—"}</strong></div>
              <button type="button" className="calculatorDone" onClick={() => setCalculatorOpen(false)}>არჩევის დასრულება</button>
              </div>
            </section>
          </div>}
