...
export default function SubscriptionsPage() {
  ...
  return <main className="subscriptionsPage" data-app-step={appStep}>
      ...
      <div className="layout">
        <section className="panel">
          ...
          <section className="liveCalculator" aria-label="ფასის კალკულატორი">
            ...
            <details className="priceInstructions"><summary><span>ყველა პროდუქტის ფასი</span><b>ვადის არჩევა ⌄</b></summary><div className="priceCalendar"><div className="priceCalendarHead"><span>პროფილი</span>{PERIODS.map((item) => <b key={item.value}>{item.label}</b>)}</div>{PRODUCTS.map((product) => { const chosen = selectedItems.filter((profile) => normalizeType(profile) === product.type); if (!chosen.length) return <article key={product.type} className="referencePrice"><span><i>{product.icon}</i><strong>{product.name}</strong></span>{PERIODS.map((item) => <b key={item.value}>{product.prices[item.value]}₾</b>)}</article>; return <section key={product.type} className="chosenProductGroup"><header><span>{product.icon} {product.name}</span><b>{chosen.length} პროფილი</b></header>{chosen.map((profile) => { const ownPeriod = selectedPeriod(profile.id); return <article key={profile.id} className="chosenProduct"><span><strong>{profile.item_name || profile.tag_code}</strong><small>{profile.tag_code}</small></span>{PERIODS.map((item) => { const active = ownPeriod === item.value; return <button key={item.value} type="button" className={active ? "active" : ""} onClick={() => setProfilePeriods((current) => ({ ...current, [profile.id]: item.value }))}><span>{product.prices[item.value]}₾</span>{active && <i>✓</i>}</button>; })}</article>; })}</section>; })}<p><strong>როგორ ავირჩიოთ:</strong> მონიშნეთ პროფილები ქვემოთ. კალენდარში თითოეული პროფილი ცალკე ხაზად გამოჩნდება და ყველას საკუთარ ვადას მიანიჭებთ.</p><p><strong>რაოდენობრივი ფასდაკლება:</strong> 3–4 პროფილი −5% · 5–6 −10% · 7 −12% · 8 ან მეტი −15%</p></div></details>
          </section>
    ...
    <style jsx>{`
      ...
      .profilePeriodPicker{display:none}.chosen span small{display:block;margin-top:3px;color:inherit;font-size:9px;opacity:.72}.chosenProductGroup{margin-top:7px;padding:6px;border:1px solid #0aa26b;border-radius:12px;background:#edfbf5}.chosenProductGroup>header{padding:2px 4px 6px;display:flex;align-items:center;justify-content:space-between;color:#167054;font-size:9px;font-weight:900}.chosenProductGroup>header b{padding:3px 6px;border-radius:99px;background:#d7f5e8;font-size:7px}.chosenProductGroup .chosenProduct{margin-top:4px}.chosenProductGroup .chosenProduct>span{grid-template-columns:1fr}.chosenProductGroup .chosenProduct>span strong,.chosenProductGroup .chosenProduct>span small{display:block}.chosenProductGroup .chosenProduct>span small{margin-top:2px;overflow:hidden;color:#71889a;font-size:7px;text-overflow:ellipsis;white-space:nowrap}.priceCalendar .referencePrice>b{text-align:center;color:#0870d8;font-size:10px}
