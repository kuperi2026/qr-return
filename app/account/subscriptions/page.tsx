      ...
      <div className="layout">
        <section className="panel">
          ...
          <section className="liveCalculator" aria-label="ფასის კალკულატორი">
            ...
            <details className="priceInstructions"><summary><span>ყველა პროდუქტის ფასი</span><b>ვადის არჩევა ⌄</b></summary><div className="priceCalendar"><div className="priceCalendarHead"><span>პროდუქტი</span>{PERIODS.map((item) => <b key={item.value}>{item.label}</b>)}</div>{PRODUCTS.map((product) => { const chosen = selectedItems.filter((profile) => normalizeType(profile) === product.type); return <article key={product.type} className={chosen.length ? "chosenProduct" : ""}><span><i>{product.icon}</i><strong>{product.name}</strong>{chosen.length > 0 && <em>✓ {chosen.length}</em>}</span>{PERIODS.map((item) => { const active = chosen.length > 0 && chosen.every((profile) => selectedPeriod(profile.id) === item.value); return <button key={item.value} type="button" disabled={!chosen.length} className={active ? "active" : ""} onClick={() => setProfilePeriods((current) => ({ ...current, ...Object.fromEntries(chosen.map((profile) => [profile.id, item.value])) }))}><span>{product.prices[item.value]}₾</span>{active && <i>✓</i>}</button>; })}</article>; })}<p><strong>როგორ ავირჩიოთ:</strong> ჯერ მონიშნეთ პროფილი ქვემოთ, შემდეგ მის მწვანედ მონიშნულ რიგში დააჭირეთ სასურველ ფასს.</p><p><strong>რაოდენობრივი ფასდაკლება:</strong> 3–4 პროფილი −5% · 5–6 −10% · 7 −12% · 8 ან მეტი −15%</p></div></details>
          </section>
          {loading ? <div className="state">პროფილები იტვირთება...</div> : profiles.length ? <div className="profilePicker">
            ...
            {profileGroups.map((group) => {
              ...
              <div className="profiles">{group.profiles.map((profile) => {
                ...
                return <article key={profile.id} className={selected ? "profileOption selected" : "profileOption"}><button className={selected ? "profile selected" : "profile"} onClick={() => toggleProfile(profile.id)}><span className="icon">{meta?.icon || "🏷️"}</span><span><b>{profile.item_name || meta?.name || "QR პროფილი"}</b><small>{meta?.name} · {profile.tag_code}</small></span><span className="profilePrice">{selected ? `${meta?.prices[ownPeriod] || 0} ₾` : ""}<small>{selected ? PERIODS.find((item) => item.value === ownPeriod)?.label : ""}</small></span><i>{selected ? "✓" : ""}</i></button>{selected && !isAppPricing && <div className="profileTerms"><div className="desktopProfileTerm"><span>არჩეული ვადა</span><b>{PERIODS.find((item) => item.value === ownPeriod)?.label}</b></div><div className="desktopProfileTerm"><span>პროფილის ფასი</span><b>{meta?.prices[ownPeriod] || 0} ₾</b></div><p className="repeatedFreePeriod">QR კოდის გააქტიურების შემდეგ მომსახურებით სარგებლობა 60 დღის განმავლობაში უფასოა.</p></div>}</article>;
              })}</div>
            </section>;})}
