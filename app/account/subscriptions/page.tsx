...
type Profile = { id: string; tag_code: string; item_name: string | null; item_type: string | null; pet_type: string | null; created_at?: string | null };
...
export default function SubscriptionsPage() {
  ...
  useEffect(() => { void (async () => {
    ...
    const rows = (data || []) as Profile[]; setProfiles(rows); setSelectedProfile(rows[0]?.id || ""); setLoading(false);
  })(); }, [router]);

  const activeProfile = profiles.find((item) => item.id === selectedProfile) || null;
  const product = PRODUCTS.find((item) => item.type === (activeProfile ? normalizeType(activeProfile) : "")) || null;
  const total = product?.prices[period] || 0;
      ...
      <div className="layout">
        <section className="panel">
          ...
          {loading ? <div className="state">პროფილები იტვირთება...</div> : profiles.length ? <div className="profiles">{profiles.map((profile) => {
            ...
            return <button key={profile.id} className={selectedProfile === profile.id ? "profile selected" : "profile"} onClick={() => setSelectedProfile(profile.id)}><span className="icon">{meta?.icon || "🏷️"}</span><span><b>{profile.item_name || meta?.name || "QR პროფილი"}</b><small>{meta?.name} · {profile.tag_code}</small></span><i>{selectedProfile === profile.id ? "✓" : ""}</i></button>;
          })}</div> : <div className="state">ჯერ QR პროფილი არ გაქვს. <Link href="/register">პროფილის დამატება</Link></div>}

          <h2>2. აირჩიე ვადა</h2>
          <div className="periods">{PERIODS.map((item) => <button key={item.value} className={period === item.value ? "period active" : "period"} onClick={() => setPeriod(item.value)}><b>{item.label}</b><span>{product ? `${product.prices[item.value]} ₾` : "—"}</span>{item.value === "12" && <em>საუკეთესო ფასი</em>}</button>)}</div>
        </section>

        <aside className="summary">
          <small>შეკვეთის შეჯამება</small><h2>{product ? `${product.icon} ${product.name}` : "აირჩიე პროფილი"}</h2>
          <div className="line"><span>არჩეული ვადა</span><b>{PERIODS.find((item) => item.value === period)?.label}</b></div>
          <div className="line"><span>მომსახურების ღირებულება</span><b>{total ? `${total} ₾` : "—"}</b></div>
          <div className="total"><span>სულ გადასახდელი</span><strong>{total ? `${total} ₾` : "0 ₾"}</strong></div>
          <button disabled={!product} onClick={() => alert("ეს სატესტო ვერსიაა — რეალური გადახდა ჯერ არ შესრულდება.")}>პაკეტის არჩევა</button>
          <p>ეს სატესტო გვერდია. თანხა არ ჩამოგეჭრება.</p>
        </aside>
      </div>

      <section className="prices"><div><small>სრული ტარიფები</small><h2>ყველა პროდუქტის ფასი</h2></div><div className="tableWrap"><table><thead><tr><th>პროდუქტი</th>{PERIODS.map((p) => <th key={p.value}>{p.label}</th>)}</tr></thead><tbody>{PRODUCTS.map((item) => <tr key={item.type}><td><span>{item.icon}</span><b>{item.name}</b></td>{PERIODS.map((p) => <td key={p.value}>{item.prices[p.value]} ₾</td>)}</tr>)}</tbody></table></div></section>
    </section>
