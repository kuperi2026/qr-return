...
export default function Dashboard() {
  ...
  return <main className="simpleDash"><div className="dashWrap">
    ...
    <section className="welcome"><span className="welcomeIcon">✦</span><div><small>KOMPASI CONTROL</small><h1>დაცული QR სივრცე</h1><p>პროფილები, შემთხვევები და უსაფრთხო კავშირი ერთ სივრცეში.</p></div></section>
    ...
    <p className="hint"><span>＋</span> პროფილის დასამატებლად ან სამართავად ქვედა მენიუში გახსენით „ჰაბი“.</p>
    <section className="compactActions"><Link href="/app/chat"><i>◌</i><span><b>Live Chat Inbox</b><small>ყველა მპოვნელთან ცალკე საუბარი</small></span><em>›</em></Link><button type="button" onClick={() => setShowAnalysis((value) => !value)}><i>✦</i><span><b>AI ანალიზი</b><small>სკანები, Lost Mode და უსაფრთხოების რჩევები</small></span><em>{showAnalysis ? "⌃" : "⌄"}</em></button></section>
    {showAnalysis && <section className="analysisCard"><small>KOMPASI AI INSIGHT</small><h2>{lost ? `${lost} პროფილს ახლა ყურადღება სჭირდება` : "პროფილები დაცულ მდგომარეობაშია"}</h2><p>{lost ? "Lost Mode ჩართულია — შეამოწმეთ ბოლო სკანირება და Live Chat-ის ახალი საუბრები." : scans ? `${scans} სკანირება დაფიქსირდა. გახსენით პროფილების მართვა ბოლო მდებარეობებისა და აქტივობის სანახავად.` : "პირველი სკანირების შემდეგ აქ გამოჩნდება აქტივობის ანალიზი და რეკომენდაციები."}</p><div><span>პროფილები <b>{items.length}</b></span><span>სკანები <b>{scans}</b></span><span>შემთხვევა <b>{lost}</b></span></div></section>}
  ...
  `}</style><style jsx global>{`
    .compactActions button{width:100%;min-height:67px;padding:11px 13px;display:flex;align-items:center;gap:11px;border:1px solid #dce6f0;border-radius:15px;background:#fff;color:#173652;text-align:left;font-family:inherit;cursor:pointer;box-shadow:0 8px 22px rgba(1,30,66,.14)}
    .compactActions button i{background:#f0eaff!important;color:#6847c6!important}.compactActions button span{min-width:0;flex:1}.compactActions button b,.compactActions button small{display:block}.compactActions button b{font-size:13px}.compactActions button small{margin-top:4px;color:#74899d;font-size:10px}.compactActions button em{color:#6847c6;font-size:20px;font-style:normal}
    .analysisCard{margin-top:10px;padding:17px;border:1px solid rgba(255,255,255,.72);border-radius:18px;background:linear-gradient(145deg,#f0eaff,#edf7ff 55%,#e9fff7);color:#173652;box-shadow:0 14px 30px rgba(1,30,66,.2)}.analysisCard>small{color:#6847c6;font-size:9px;font-weight:950;letter-spacing:1px}.analysisCard h2{margin:7px 0;font-size:18px}.analysisCard p{margin:0;color:#587087;font-size:11px;line-height:1.55}.analysisCard>div{margin-top:13px;display:grid;grid-template-columns:repeat(3,1fr);gap:6px}.analysisCard>div span{padding:9px 5px;border-radius:10px;background:rgba(255,255,255,.8);font-size:8px;text-align:center}.analysisCard>div b{display:block;margin-top:3px;color:#075dcc;font-size:15px}
  `}</style></main>;
}
