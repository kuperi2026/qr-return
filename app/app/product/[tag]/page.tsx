...
export default function ProductProfile() {
  ...
  const readinessValues = [item.item_name, item.photo, item.description, item.finder_message, item.owner_phone, ...details.map(([, value]) => value)];
  const filledReadiness = readinessValues.filter(Boolean).length;
  const visibilityCount = [item.show_pet_photo, item.show_owner_name, item.show_owner_phone, item.show_email, item.show_description, item.show_finder_message, item.phone_enabled, item.live_chat_enabled].filter(Boolean).length;
  const profileSignals = { completeness: Math.round((filledReadiness / readinessValues.length) * 100), missingCount: readinessValues.length - filledReadiness, visibilityCount, lostMode: Boolean(item.lost), liveChat: Boolean(item.live_chat_enabled), scanCount: item.scan_count || 0 };
