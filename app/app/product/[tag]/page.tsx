...
export default function ProductProfile() {
  ...
  return <main className="productProfile"><div className="profileWrap">
    <header className="topLine"><Link href={`/app/products/${type}`}>← {meta[1]}ები</Link><button type="button" onClick={() => { setEditing(!editing); setDraft(item); setMessage(""); }}>{editing ? "გაუქმება" : "რედაქტირება"}</button></header>
    ...
    <nav className="quickNav" aria-label="პროფილის სექციები"><a href="#info">ინფორმაცია</a><a href="#lost">Lost Mode</a><a href="#scans">სკანები</a><a href="#contact">კავშირი</a></nav>
    {message && <p className={message.includes("შენახულია") ? "notice success" : "notice"}>{message}</p>}

    {editing ? <section className="section editSection" id="info">
    ...
    </section> : <section className="section" id="info">
    ...
    <section className={`section lostSection ${item.lost ? "isLost" : ""}`} id="lost"><Title number="02" title="Lost Mode" text={item.lost ? `ჩართულია ${formatDate(item.lost_at)}` : "დაკარგვის შემთხვევაში ჩართეთ ერთი დაჭერით"} /><button type="button" disabled={saving} onClick={toggleLost}>{item.lost ? "ნაპოვნია — რეჟიმის გამორთვა" : "Lost Mode-ის ჩართვა"}</button></section>
    <AppAiAssistant />
    <section className="section" id="scans"><Title number="03" title="სკანირებები და მდებარეობა" text="როდის და სად დაასკანირეს QR" /><div className="scanStats"><div><span>სულ</span><b>{item.scan_count || 0}</b></div><div><span>ბოლო სკანირება</span><b>{formatDate(item.last_scanned_at)}</b></div></div>{hasLocation && <a className="mapLink" href={`https://www.google.com/maps?q=${item.last_scan_latitude},${item.last_scan_longitude}`} target="_blank" rel="noreferrer">⌖ ბოლო მდებარეობის რუკაზე ნახვა ↗</a>}<button className="historyButton" type="button" onClick={loadHistory}>{historyLoaded ? "ისტორია განახლებულია" : "სკანირების ისტორიის ნახვა"}</button>{historyLoading && <p className="muted">ისტორია იტვირთება…</p>}{historyLoaded && <div className="history">{history.length ? history.map((event) => <div key={event.id}><span>{formatDate(event.created_at)}</span>{event.latitude != null && event.longitude != null ? <a href={`https://www.google.com/maps?q=${event.latitude},${event.longitude}`} target="_blank" rel="noreferrer">რუკა ↗</a> : <small>ლოკაცია არ გაზიარებულა</small>}</div>) : <p className="muted">სკანირება ჯერ არ ყოფილა.</p>}</div>}</section>
    <section className="section" id="contact"><Title number="04" title="კავშირი და QR" text="მპოვნელთან საუბარი და პროფილის მართვა" /><div className="mainActions"><Link className="chat" href={`/chat/${type}/${tag}`}>Live Chat-ის გახსნა</Link><Link href={`/scan/${tag}`} target="_blank">მპოვნელის პროფილი ↗</Link><button type="button" onClick={downloadQR}>QR კოდის ჩამოტვირთვა</button><Link href={`/account/admin?profile=${item.id}`}>თანაადმინისტრატორი</Link></div></section>
    <section className="serviceLine"><div><span>მომსახურება</span><b>{item.service_status === "active" ? "აქტიური პაკეტი" : item.service_status === "expired" ? "ვადა გასულია" : "საცდელი პერიოდი"}</b><small>{formatDate(item.service_expires_at || item.trial_ends_at)}</small></div><Link href={`/account/subscriptions?profile=${item.id}`}>პაკეტის მართვა →</Link></section>
