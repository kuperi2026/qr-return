...
export default function ProductProfile() {
  ...
  return <main className="productProfile"><div className="profileWrap">
    ...
    <details className="accordion" open={openPanel === "profile"}><summary onClick={(event) => { event.preventDefault(); setOpenPanel((current) => current === "profile" ? null : "profile"); }}><span>▤</span><div><b>პროფილის ინფორმაცია და რედაქტირება</b><small>მონაცემები და ხილვადობა</small></div><em>⌄</em></summary>
    ...
    <details className={`accordion ${item.lost ? "danger" : ""}`} open={openPanel === "lost"}><summary onClick={(event) => { event.preventDefault(); setOpenPanel((current) => current === "lost" ? null : "lost"); }}><span>!</span><div><b>Lost Mode</b><small>{item.lost ? "ჩართულია" : "დაკარგვის რეჟიმი"}</small></div><em>⌄</em></summary><section className={`section lostSection ${item.lost ? "isLost" : ""}`}><Title number="02" title="Lost Mode" text={item.lost ? `ჩართულია ${formatDate(item.lost_at)}` : "დაკარგვის შემთხვევაში ჩართეთ ერთი დაჭერით"} /><button type="button" disabled={saving} onClick={toggleLost}>{item.lost ? "ნაპოვნია — რეჟიმის გამორთვა" : "Lost Mode-ის ჩართვა"}</button></section></details>
    <details className="accordion" open={openPanel === "ai"}><summary onClick={(event) => { event.preventDefault(); setOpenPanel((current) => current === "ai" ? null : "ai"); }}><span>✦</span><div><b>AI დამხმარე</b><small>კითხვა და დახმარება</small></div><em>⌄</em></summary><div className="aiPanel"><AppAiAssistant /></div></details>
