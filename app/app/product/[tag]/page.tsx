...
type ServiceWindow = { status: string; service_starts_at: string | null; service_expires_at: string | null };
...
const FEATURES: Record<string, { info: string; lost: string; scans: string; contact: string; ai: string }> = {
  dog: { info: "ძაღლის მონაცემები და ჯანმრთელობა", lost: "დაკარგული ძაღლის რეჟიმი", scans: "სკანები და ბოლო მდებარეობა", contact: "მპოვნელთან კავშირი და QR", ai: "AI ძაღლის მოძებნის დამხმარე" },
  cat: { info: "კატის მონაცემები და ჯანმრთელობა", lost: "დაკარგული კატის რეჟიმი", scans: "სკანები და ბოლო მდებარეობა", contact: "მპოვნელთან კავშირი და QR", ai: "AI კატის მოძებნის დამხმარე" },
  parking: { info: "ავტომობილის ინფორმაცია", lost: "ავტომობილის უსაფრთხოების რეჟიმი", scans: "QR შეტყობინებები და მდებარეობა", contact: "მძღოლთან უსაფრთხო კავშირი", ai: "AI Parking დამხმარე" },
  keys: { info: "გასაღების აღწერა", lost: "დაკარგული გასაღების რეჟიმი", scans: "პოვნის სკანები და მდებარეობა", contact: "მპოვნელთან კავშირი და QR", ai: "AI დაბრუნების დამხმარე" },
  wallet: { info: "საფულის აღწერა და კონფიდენციალურობა", lost: "დაკარგული საფულის რეჟიმი", scans: "პოვნის სკანები და მდებარეობა", contact: "უსაფრთხო კავშირი და QR", ai: "AI უსაფრთხო დაბრუნების დამხმარე" },
  bag: { info: "ჩანთის აღწერა", lost: "დაკარგული ჩანთის რეჟიმი", scans: "პოვნის სკანები და მდებარეობა", contact: "მპოვნელთან კავშირი და QR", ai: "AI ჩანთის დაბრუნების დამხმარე" },
  suitcase: { info: "ჩემოდნის აღწერა და ნიშნები", lost: "დაკარგული ჩემოდნის რეჟიმი", scans: "მგზავრობის სკანები და მდებარეობა", contact: "მპოვნელთან კავშირი და QR", ai: "AI მოგზაურობის დამხმარე" },
};
...
export default function ProductProfile() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setItem(data as Item | null); setDraft(data as Item | null);
      if (data?.id) {
        const { data: service } = await supabase.from("service_activation_requests").select("status,service_starts_at,service_expires_at").eq("owner_id", user.id).eq("item_id", data.id).order("requested_at", { ascending: false }).limit(1).maybeSingle();
        setServiceWindow(service as ServiceWindow | null);
      }
      setLoading(false);
    })();
  }, [router, tag]);
  ...
  const feature = FEATURES[type] || { info: "პროფილის ინფორმაცია", lost: "Lost Mode", scans: "სკანები და მდებარეობა", contact: "კავშირი და QR", ai: "AI დამხმარე" };
  ...
  async function deleteProfile() {
    if (deleting || !window.confirm("პროფილი და მისი სკანირების ისტორია სამუდამოდ წაიშლება. ნამდვილად გსურთ გაგრძელება?")) return;
    setDeleting(true); setMessage("");
    const { data, error } = await sb!.rpc("delete_owned_item", { p_item_id: Number(item!.id) });
    if (error || !data) { setMessage(error?.message || "პროფილის წაშლა ვერ მოხერხდა."); setDeleting(false); return; }
  ...
  return <main className="productProfile"><div className="profileWrap">
    ...
    {editing ? <section className="section editSection">
      ...
      <Title number="01" title="პროფილის სრული ინფორმაცია" text="რეგისტრირებული მონაცემები და მფლობელის საკონტაქტო ინფორმაცია" />
    ...
    <details className={`accordion ${item.lost ? "danger" : ""}`} open={openPanel === "lost"}><summary onClick={(event) => { event.preventDefault(); setOpenPanel((current) => current === "lost" ? null : "lost"); }}><span>!</span><div><b>{feature.lost}</b><small>{item.lost ? "ჩართულია" : "ერთი დაჭერით ჩართვა"}</small></div><em>⌄</em></summary><section className={`section lostSection ${item.lost ? "isLost" : ""}`}><Title number="02" title="Lost Mode" text={item.lost ? `ჩართულია ${formatDate(item.lost_at)}` : "დაკარგვის შემთხვევაში ჩართეთ ერთი დაჭერით"} /><button type="button" disabled={saving} onClick={toggleLost}>{item.lost ? "ნაპოვნია — რეჟიმის გამორთვა" : "Lost Mode-ის ჩართვა"}</button></section></details>
    <details className="accordion" open={openPanel === "ai"}><summary onClick={(event) => { event.preventDefault(); setOpenPanel((current) => current === "ai" ? null : "ai"); }}><span>✦</span><div><b>{feature.ai}</b><small>ამ პროფილზე მორგებული დახმარება</small></div><em>⌄</em></summary><div className="aiPanel"><AppAiAssistant category={type} profileName={item.item_name} embedded /></div></details>
    <details className="accordion" open={openPanel === "scans"}><summary onClick={(event) => { event.preventDefault(); setOpenPanel((current) => current === "scans" ? null : "scans"); }}><span>⌖</span><div><b>სკანირებები და მდებარეობა</b><small>{item.scan_count || 0} სკანირება</small></div><em>⌄</em></summary><section className="section"><Title number="03" title="სკანირებები და მდებარეობა" text="როდის და სად დაასკანირეს QR" /><div className="scanStats"><div><span>სულ</span><b>{item.scan_count || 0}</b></div><div><span>ბოლო სკანირება</span><b>{formatDate(item.last_scanned_at)}</b></div></div>{hasLocation && <a className="mapLink" href={`https://www.google.com/maps?q=${item.last_scan_latitude},${item.last_scan_longitude}`} target="_blank" rel="noreferrer">⌖ ბოლო მდებარეობის რუკაზე ნახვა ↗</a>}<button className="historyButton" type="button" onClick={loadHistory}>{historyLoaded ? "ისტორია განახლებულია" : "სკანირების ისტორიის ნახვა"}</button>{historyLoading && <p className="muted">ისტორია იტვირთება…</p>}{historyLoaded && <div className="history">{history.length ? history.map((event) => <div key={event.id}><span>{formatDate(event.created_at)}</span>{event.latitude != null && event.longitude != null ? <a href={`https://www.google.com/maps?q=${event.latitude},${event.longitude}`} target="_blank" rel="noreferrer">რუკა ↗</a> : <small>ლოკაცია არ გაზიარებულა</small>}</div>) : <p className="muted">სკანირება ჯერ არ ყოფილა.</p>}</div>}</section></details>
    <details className="accordion" open={openPanel === "contact"}><summary onClick={(event) => { event.preventDefault(); setOpenPanel((current) => current === "contact" ? null : "contact"); }}><span>◌</span><div><b>კავშირი და QR</b><small>ჩათი, QR კოდი და სანდო პირი</small></div><em>⌄</em></summary><section className="section"><Title number="04" title="კავშირი და QR" text="მპოვნელთან საუბარი და პროფილის მართვა" /><div className="mainActions"><Link className="chat" href={`/chat/${type}/${tag}`}>ჩათის გახსნა</Link><Link href={`/scan/${tag}`} target="_blank">მპოვნელის პროფილი ↗</Link><button type="button" onClick={downloadQR}>QR კოდის ჩამოტვირთვა</button><Link href={`/account/admin?profile=${item.id}`}>სანდო პირის მართვა</Link></div></section></details>
    <details className="accordion" open={openPanel === "service"}><summary onClick={(event) => { event.preventDefault(); setOpenPanel((current) => current === "service" ? null : "service"); }}><span>◇</span><div><b>მომსახურების სტატუსი</b><small>{item.service_status === "active" ? "აქტიურია" : item.service_status === "expired" ? "ვადა გასულია" : "საცდელი პერიოდი"}</small></div><em>⌄</em></summary><section className="serviceStatus"><div className={`serviceBadge ${item.service_status === "active" ? "active" : item.service_status === "expired" ? "expired" : "trial"}`}><i>●</i><span><small>მდგომარეობა</small><b>{item.service_status === "active" ? "აქტიური მომსახურება" : item.service_status === "expired" ? "მომსახურების ვადა გასულია" : "უფასო პერიოდი"}</b></span></div><div className="serviceDates"><div><small>დაწყების თარიღი</small><b>{serviceWindow?.service_starts_at ? formatDate(serviceWindow.service_starts_at) : "გააქტიურების შემდეგ"}</b></div><div><small>დასრულების თარიღი</small><b>{formatDate(serviceWindow?.service_expires_at || item.service_expires_at || item.trial_ends_at)}</b></div></div><Link className="extendService" href={`/account/subscriptions?source=app&profile=${item.id}`}>ვადის გაგრძელება <span>→</span></Link></section></details>
    </div>
    <section className="deleteZone"><div><b>პროფილის წაშლა</b><small>ეს მოქმედება შეუქცევადია და წაშლის პროფილის ისტორიასაც.</small></div><button type="button" disabled={deleting} onClick={deleteProfile}>{deleting ? "იშლება…" : "სამუდამოდ წაშლა"}</button></section>
...
function ProductStyles() {
  return <style jsx global>{`
    *{box-sizing:border-box}.productState{min-height:100vh;display:grid;place-items:center;background:#f4f7fb;color:#6e8296;font:700 12px "Noto Sans Georgian","Sylfaen",Inter,Arial}.productProfile{min-height:100vh;background:linear-gradient(180deg,#edf5fb,#f7f9fc 44%,#f0f8f5);color:#173652;font-family:"Noto Sans Georgian","Sylfaen",Inter,Arial,sans-serif}.profileWrap{width:min(560px,100%);margin:auto;padding:16px 14px 100px}.topLine{display:flex;align-items:center;justify-content:space-between}.topLine a{color:#1761bd;text-decoration:none;font-size:10px;font-weight:850}.topLine button{padding:8px 12px;border:1px solid #cbdbea;border-radius:10px;background:#fff;color:#1761bd;font:850 10px inherit}.identity{margin-top:14px;padding:18px;display:flex;align-items:center;gap:14px;border-radius:20px;background:linear-gradient(140deg,#07366f,#0a65d2 62%,#079777);color:#fff;box-shadow:0 13px 30px #0b4eaa27}.photo{width:72px;height:72px;display:grid;place-items:center;flex:0 0 72px;overflow:hidden;border:2px solid #ffffff55;border-radius:20px;background:#ffffff18;font-size:30px}.photo img{width:100%;height:100%;object-fit:cover}.identity small{color:#bcdafa;font-size:8px}.identity h1{margin:5px 0 8px;font-size:22px}.identity b{padding:4px 8px;border-radius:999px;font-size:8px}.identity .active{background:#dff7eb;color:#087649}.identity .lost{background:#ffe6e8;color:#b82937}.quickNav{margin:12px 0 4px;display:flex;gap:7px;overflow:auto;scrollbar-width:none}.quickNav a{flex:0 0 auto;padding:8px 10px;border:1px solid #dce6f0;border-radius:999px;background:#fff;color:#49647e;text-decoration:none;font-size:8px;font-weight:850}.section{padding:22px 2px;border-bottom:1px solid #dce5ee;scroll-margin-top:16px}.sectionTitle{display:flex;align-items:flex-start;gap:10px}.sectionTitle>span{width:29px;height:29px;display:grid;place-items:center;flex:0 0 29px;border-radius:9px;background:#e7f1ff;color:#1761bd;font-size:8px;font-weight:900}.sectionTitle h2{margin:0;font-size:15px}.sectionTitle p{margin:4px 0 0;color:#75899d;font-size:9px}.infoRows{margin-top:16px;display:grid;grid-template-columns:1fr 1fr;gap:0 18px}.infoRows div{padding:11px 0;border-bottom:1px solid #e5ebf1}.infoRows span,.infoRows b{display:block}.infoRows span{color:#8394a4;font-size:8px}.infoRows b{margin-top:4px;font-size:11px}.notes{margin-top:12px}.notes p{margin:0;padding:11px 0;border-bottom:1px solid #e5ebf1;color:#465f76;font-size:10px;line-height:1.5}.notes span{display:block;margin-bottom:4px;color:#1761bd;font-size:8px;font-weight:900}.owner{margin-top:14px;padding:13px 0}.owner span,.owner b,.owner small{display:block}.owner span{color:#8394a4;font-size:8px}.owner b{margin-top:4px;font-size:12px}.owner small{margin-top:4px;color:#657b90;font-size:9px}.formGrid{margin-top:16px;display:grid;grid-template-columns:1fr 1fr;gap:10px}.formGrid label span{display:block;margin-bottom:5px;color:#5e7489;font-size:8px;font-weight:850}.formGrid input,.formGrid textarea{width:100%;padding:11px;border:1px solid #d5e1ec;border-radius:10px;background:#fff;color:#173652;font:500 11px inherit}.formGrid textarea{min-height:76px;resize:vertical}.formGrid .wide{grid-column:1/-1}.visibility{margin-top:18px;display:grid;grid-template-columns:1fr 1fr;gap:8px}.visibility h3{grid-column:1/-1;margin:0 0 4px;font-size:11px}.switch{min-height:39px;padding:8px 9px;display:flex;align-items:center;justify-content:space-between;gap:7px;border:1px solid #dce6f0;border-radius:10px;background:#fff}.switch span{font-size:8px;font-weight:800}.switch input{accent-color:#176be5}.save{width:100%;min-height:45px;margin-top:14px;border:0;border-radius:11px;background:#1266e9;color:#fff;font:900 10px inherit}.lostSection button{width:100%;min-height:44px;margin-top:15px;border:0;border-radius:11px;background:#1266e9;color:#fff;font:900 10px inherit}.lostSection.isLost .sectionTitle>span{background:#ffe5e7;color:#bb2733}.lostSection.isLost button{border:1px solid #e9aeb4;background:#fff;color:#b4232c}.scanStats{margin-top:15px;display:grid;grid-template-columns:.7fr 1.5fr;gap:9px}.scanStats div{padding:12px 0}.scanStats span,.scanStats b{display:block}.scanStats span{color:#8394a4;font-size:8px}.scanStats b{margin-top:5px;color:#155db5;font-size:11px}.mapLink{display:block;margin-top:4px;padding:11px;border-radius:10px;background:#eaf3ff;color:#075dcc;text-decoration:none;font-size:9px;font-weight:850}.historyButton{margin-top:9px;padding:9px 11px;border:1px solid #d5e1ec;border-radius:9px;background:#fff;color:#49647e;font:850 9px inherit}.history{margin-top:9px}.history>div{min-height:38px;padding:8px 2px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #e5ebf1;font-size:8px}.history a{color:#1761bd;text-decoration:none;font-weight:850}.history small,.muted{color:#8394a4;font-size:8px}.mainActions{margin-top:15px;display:grid;grid-template-columns:1fr 1fr;gap:8px}.mainActions a,.mainActions button{min-height:43px;padding:8px;display:flex;align-items:center;justify-content:center;border:1px solid #d5e1ec;border-radius:10px;background:#fff;color:#1761bd;text-align:center;text-decoration:none;font:850 9px inherit}.mainActions .chat{border-color:#176be5;background:#1266e9;color:#fff}.serviceStatus{padding:15px}.serviceBadge{padding:12px;display:flex;align-items:center;gap:10px;border:1px solid #d7e5f3;border-radius:12px;background:#f6faff}.serviceBadge>i{color:#e3a323;font-size:11px;font-style:normal}.serviceBadge.active>i{color:#0a9b64}.serviceBadge.expired>i{color:#cc3340}.serviceBadge span,.serviceBadge small,.serviceBadge b{display:block}.serviceBadge span{min-width:0}.serviceBadge small{color:#7890a5;font-size:9px}.serviceBadge b{margin-top:3px;color:#173652;font-size:12px}.serviceDates{margin-top:9px;display:grid;grid-template-columns:1fr 1fr;gap:8px}.serviceDates>div{padding:11px;border:1px solid #dce7f1;border-radius:11px;background:#fff}.serviceDates small,.serviceDates b{display:block}.serviceDates small{color:#8093a5;font-size:8px}.serviceDates b{margin-top:5px;font-size:10px;line-height:1.35}.extendService{min-height:43px;margin-top:10px;padding:0 13px;display:flex;align-items:center;justify-content:space-between;border-radius:11px;background:#1266e9;color:#fff;text-decoration:none;font-size:11px;font-weight:900}.notice{margin:12px 0 0;padding:10px;border-radius:10px;background:#fff1f1;color:#a52630;font-size:9px;font-weight:800}.notice.success{background:#e8f8f0;color:#087649}@media(max-width:380px){.formGrid,.visibility,.mainActions,.serviceDates{grid-template-columns:1fr}.formGrid .wide,.visibility h3{grid-column:1}.infoRows{grid-template-columns:1fr}}
    ...
    .deleteZone{margin-top:14px;padding:15px;display:flex;align-items:center;justify-content:space-between;gap:12px;border:1px solid #efc4c8;border-radius:15px;background:#fff6f6}.deleteZone b,.deleteZone small{display:block}.deleteZone b{color:#a6232d;font-size:12px}.deleteZone small{margin-top:4px;color:#93676b;font-size:8px;line-height:1.4}.deleteZone button{min-height:40px;padding:0 11px;flex:0 0 auto;border:0;border-radius:10px;background:#b4232d;color:#fff;font:900 9px inherit}.deleteZone button:disabled{opacity:.55}
  `}</style>;
}
