...
export default function EmergencyAppProfile() {
  ...
  const readinessValues = [profile.first_name, profile.last_name, profile.date_of_birth, profile.blood_type, profile.allergies || profile.medical_conditions || profile.medications, profile.emergency_contact_name, profile.emergency_contact_phone, profile.emergency_message];
  const filledReadiness = readinessValues.filter(Boolean).length;
  const visibilityCount = [profile.active, profile.live_chat_enabled, profile.location_sharing_enabled].filter(Boolean).length;
  const profileSignals = { completeness: Math.round((filledReadiness / readinessValues.length) * 100), missingCount: readinessValues.length - filledReadiness, visibilityCount, lostMode: Boolean(profile.missing_mode), liveChat: Boolean(profile.live_chat_enabled), scanCount: 0 };
  ...
  return <main className="emApp"><div className="emWrap">
    ...
    <div className="emPanels">
      ...
      <Panel open={open} onToggle={toggle} name="ai" icon="AI" title="Emergency AI დამხმარე" subtitle="უსაფრთხო შეფასება პირველი და მესამე პირისთვის"><AppAiAssistant category="emergency" profileName={holderName} subjectMode={isThirdParty ? "other" : "self"} profileSignals={profileSignals} embedded /></Panel>
